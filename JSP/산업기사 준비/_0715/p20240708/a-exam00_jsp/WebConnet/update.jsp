...
<%
String sql = "select custon, custname, phone, address, "
			+" to_char(joindate,'yyyy-mm-dd') joindate, grade, city "
			+" from member_tbl_02 "
			+" where custon=" + request.getParameter("custon");
	...
	<script type="text/javascript">
		function checkValue() {
			...
			if(!cv.custname.value){
				...
				cv.grade.focus();
				return false;
			} else if(!cv.data.city.value){
				alert("도시코드를 입력하세요.");
				cv.city.focus();
				return false;
			}
		}
		function checkDel(custon) {
			mag= "삭제하시겠습니까?";
			if(confirm(msg)!=0){
				alert("삭제");
				location.href="delete.jsp?custon"=custon;
			} else {
				alert("취소");
			}
		}
	...
	<main id="section">
	   ...
	   <form name="data" action="update_p.jsp" method="post" onsubmit="return checkValue()">
	      <table class="table_line">
	         <tr>
	            <th>회원번호</th>
	            <td><input type="text" name="custno" value="<%= rs.getString("custon")%>" size="10" readonly></td>      
	         </tr>
	         <tr>
	            <th>회원성명</th>
	         ...
	         <tr>
	         	<td colspan="2" align="center">
					...
					<input type="button" name="button" value="조회"
